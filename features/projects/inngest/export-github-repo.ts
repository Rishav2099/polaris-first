...
export const exportToGithub = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      const rawMessage = event.data.error.message;

      // Extract the meaningful part from GitHub's error
      let userFriendlyMessage = "Something went wrong. Please try again.";

      if (rawMessage.includes("name already exists")) {
        userFriendlyMessage =
        ...
        userFriendlyMessage =
          "Your GitHub token is invalid. Please reconnect your account.";
      } else if (rawMessage.includes("No files to export")) {
        userFriendlyMessage = "No files found to export.";
      }
      ...
      await step.run("set-failed-status", async () => {
        await convex.mutation(api.system.updateExportStatus, {
          ...
          projectId,
          status: "failed",
          exportError: userFriendlyMessage,
        });
      });
    },
  },
