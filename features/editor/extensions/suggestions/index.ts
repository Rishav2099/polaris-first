...
class SuggestionWidget extends WidgetType {
  ...
  toDOM() {
    ...
    ((span.style.opacity = "0.4"), (span.style.pointerEvents = "none"));
    return span;
  }
}
...
const generatePayload = (view: EditorView, fileName: string) => {
  ...
  const linesToFetch = Math.max(5, totalLines - currentLine.number);
...
const renderPlugin = ViewPlugin.fromClass(
  class {
    ...
    update(update: ViewUpdate) {
      // Rebuild decorations if doc change, cursor moved, or suggestion changed
    ...
    build(view: EditorView) {
      ...
      // Create a wedget decoration at the cursor position
